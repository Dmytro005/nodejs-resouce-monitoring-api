...
// Reads json from given path name
helpers.readObjectFromJSON = function(pathToJson) {
    try {
        const buffer = fs.readFileSync(pathToJson, "utf-8");
        const stringifiedJSON = buffer.toString("base64");
        return JSON.parse(stringifiedJSON);
    } catch (error) {
        console.log("Some error ocuured while reading json", error);
        return {};
    }
};

// Test function for my first test in node
helpers.getANumber = () => {
    return 1;
};
