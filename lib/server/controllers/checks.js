                    ...
                    _data.read("users", phone, function(err, userData) {
                        if (!err && userData) {
                            ...
                            if (checks.length < config.maxChecks) {
                                // Verify that given url has DNS entries and therefore can resolve
                                const parsedUrl = _url.parse(
                                    protocol + "://" + url,
                                    true
                                );
                                const hostName =
                                ...
                                dns.resolve(hostName, function(err, records) {
                                    if (!err && records.length > 0) {
                                        ...
                                        const checkId = helpers.createRamdomString(
                                            20
                                        );
                                        const checkObject = {
                                            id: checkId,
                                        ...
                                        callback(400, {
                                            Error:
                                                "The host name entered didn`t resolve to any DNS entries"
                                        });
                                    }
                                });
                            } else {
